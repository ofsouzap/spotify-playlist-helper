...
def test_unique_tracks_keeps_first_occurrence_only():
    tracks = [
        TrackInfo(uri="spotify:track:1", name="A", artists=["Artist"]),
        TrackInfo(uri="spotify:track:1", name="A duplicate", artists=["Artist"]),
        TrackInfo(uri="spotify:track:2", name="B", artists=["Artist"]),
    ]
...
def test_diff_tracks_ignores_order_and_returns_set_difference():
    ...
    target_tracks = [
        TrackInfo(uri="spotify:track:2", name="B", artists=["Artist"]),
        TrackInfo(uri="spotify:track:3", name="C", artists=["Artist"]),
    ]
